      ...
      <div className="dashboard">
        ...
        <main className="main-content">
          ...
          <p className="text-muted" style={{ marginBottom: '30px' }}>
            {isAdmin() 
              ? 'Manage events and user registrations from your admin panel'
              ...
              {recentEvents.map((event) => (
                <div key={event.id} className="event-item" style={{
                  ...
                  <div>
                    ...
                    <p style={{ color: 'var(--gray-color)', fontSize: '14px' }}>
                      Date: {event.date} • Type: {event.type}
                    </p>
                  </div>
                  <span className={`badge ${event.registered ? 'badge-success' : 'badge-secondary'}`} style={{
                    padding: '5px 10px',
                    borderRadius: '20px',
                    fontSize: '12px',
                    fontWeight: '500',
                    background: event.registered ? 'var(--success-color)' : '#e9ecef',
                    color: event.registered ? 'white' : 'var(--dark-color)'
                  }}>
